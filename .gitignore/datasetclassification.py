...
df.head()legacy_df = pd.DataFrame(columns=['ImageId_ClassId', 'EncodedPixels'])
...
legacy_df.head()
df = legacy_dfdf['Image'] = df['ImageId_ClassId'].map(lambda x: x.split('_')[0])
...
plot_figures(
    ...
    bar_ylabel='Images',  labels=[' '.join((str(label), 'class(es)')) for label in unique],
...
!mkdir train_images
!mkdir val_imagesfor image_file in tqdm_notebook(X_train):
